...
class DCBlockingFilterTest : public ::testing::Test {
  protected:
    ...
    static void fillWithDC(audio::AudioBuffer<double>& buffer, double dcValue) {
    ...
    static void fillWithSine(audio::AudioBuffer<double>& buffer, double frequency, double sampleRate,
    ...
    static double measureRMS(const audio::AudioBuffer<double>& buffer) {
    ...
    static double measureMean(const audio::AudioBuffer<double>& buffer) {
        double sum = 0.0;
        int totalSamples = 0;
        for (int ch = 0; ch < buffer.getNumChannels(); ++ch) {
            for (int i = 0; i < buffer.getNumSamples(); ++i) {
...
TEST_F(DCBlockingFilterTest, Preserves20HzSine) {
    ...
    audio::AudioBuffer<double> warmupBuffer(2, 512);
    double phase = 0.0;
    ...
    audio::AudioBuffer<double> filteredBuffer(2, 512);
...
TEST_F(DCBlockingFilterTest, HarmonicPreservation) {
    ...
    // Warmup filter
    for (int block = 0; block < warmupSamples / 512; ++block) {
        audio::AudioBuffer<double> warmupBuffer(1, 512);
        for (int i = 0; i < 512; ++i) {
            double const phase = 2.0 * M_PI * fundamental * i / sampleRate;
    ...
    for (int offset = 0; offset < numSamples; offset += blockSize) {
        ...
        audio::AudioBuffer<double> block(1, samplesThisBlock);
...
TEST_F(DCBlockingFilterTest, EnabledDisabledState) {
    ...
    // Store original
    audio::AudioBuffer<double> original(2, 512);
...
TEST_F(DCBlockingFilterTest, MultiChannelProcessing) {
    ...
    audio::AudioBuffer<double> buffer(numChannels, 512);
